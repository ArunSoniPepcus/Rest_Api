...
public class EmployeeServiceImpl implements EmployeeService{
	...
	public List<Employee>findEmployeeByFilter(String name){
		...
		for(Employee employeeFindData:employees){
			if(employeeFindData.getName().equalsIgnoreCase(name) || employeeFindData.getPhoneNumber().equalsIgnoreCase(name) || employeeFindData.getEmail().equalsIgnoreCase(name)) {
				filterData.add(employeeFindData);
			}
			for(Address employeFindDataAddress: employeeFindData.getAddressList()) {
				if(employeFindDataAddress.getCity().equalsIgnoreCase(name) || employeFindDataAddress.getCountry().equalsIgnoreCase(name) || employeFindDataAddress.getState().equalsIgnoreCase(name)) {
					filterData.add(employeeFindData);
				}	
			}
		}
